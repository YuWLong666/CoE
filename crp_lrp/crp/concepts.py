...
class ChannelConcept(Concept):
    ...
    def mask(batch_id: int, concept_ids: List, layer_name=None):
        ...
        def mask_fct(grad):  # [2,4,2,2] mask others
            mask = torch.zeros_like(grad[batch_id])  # [4,2,2]
            mask[concept_ids] = 1
    ...
    def mask_rf(batch_id: int, c_n_map: Dict[int, List], layer_name=None):
        ...
        batch_id: int
            Specifies the batch dimension in the torch.Tensor.
        c_n_map: dist with int keys and list values  概念id与最大激活neuron位置 例如{168：24}
        ...
        def mask_fct(grad):
            ...
            mask = torch.zeros_like(grad[batch_id])  # torch.Size([256, 49])
    ...
    def attribute(self, relevance, mask=None, layer_name: str = None, abs_norm=True):
        ...
        rel_l = torch.sum(relevance.view(*relevance.shape[:2], -1), dim=-1)  # [1,256,7,7] --> [1,512]
    ...
    def reference_sampling(self, relevance, layer_name: str = None, max_target: str = "sum", abs_norm=True):
        ...
        if len(relevance.shape) == 3 and relevance.shape[2] == 768:
            relevance = relevance.permute(0, 2, 1)
        rel_l = relevance.view(*relevance.shape[:2], -1)  # torch.Size([20, 64, 112, 112])  --> [20,64,12544]
