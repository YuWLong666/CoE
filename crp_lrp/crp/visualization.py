...
from crp_lrp.crp.maximization import Maximization
from crp_lrp.crp.concepts import ChannelConcept, Concept
from crp_lrp.crp.statistics import Statistics
from crp_lrp.crp.hooks import FeatVisHook
from crp_lrp.crp.helper import load_maximization, load_statistics, load_stat_targets
from crp_lrp.crp.image import vis_img_heatmap, vis_opaque_img
from crp_lrp.crp.cache import Cache
...
class FeatureVisualization:
    ...
    def __init__(
            ...
            max_target="sum", abs_norm=True, path="FeatureVisualization", device=None, i_y=0, cache: Cache=None):

        if dataset.b_clip is True:
            self.b_clip = True
            self.tokenizer = dataset.tokenizer
            self.model = dataset.model
            self.preprocess = dataset.preprocess
            self.clip_classes = dataset.clip_classes
            self.text_features = dataset.text_features
    ...
    def run_distributed(self, composite: Composite, data_start, data_end, batch_size=16, checkpoint=500, on_device=None):
        ...
        for b in range(batches):
            ...
            data_batch, targets_samples = self.get_data_concurrently(samples_batch, preprocessing=True)  # [40,3,224,224]  [40]

            targets_samples = np.array(targets_samples)  # numpy operation needed  [40]
            ...
            conditions = [{self.attribution.MODEL_OUTPUT_NAME[self.i_y]: [t]} for t in targets]
    ...
    def get_max_reference(
            ...
            composite: Composite=None, rf=False, plot_fn=vis_img_heatmap, batch_size=32)-> Dict:
        """
        ...
        for c_id in concept_ids:
            ...
            d_indices_all[c_id] = d_indices

            _, label = self.get_data_concurrently(d_indices, preprocessing=False)
            d_labels_all[c_id] = label

        return ref_c, d_indices_all, d_labels_all

    @cache_reference
    def get_stats_reference(self, concept_id: int, layer_name: str, targets: Union[int, list], mode="relevance",
                            r_range: Tuple[int, int] = (0, 8), composite=None, rf=False,
                            plot_fn=vis_img_heatmap, batch_size=32):
        """
        ...
        for t in targets:
            ...
            d_c_sorted, rel_c_sorted, rf_c_sorted = load_statistics(path, layer_name, t)
    ...
    def _load_ref_and_attribution(self, d_indices, c_id, n_indices, layer_name, composite, rf, plot_fn, batch_size):
        ...
        data_batch, label = self.get_data_concurrently(d_indices, preprocessing=False)

        if composite:
            data_p = self.preprocess_data(data_batch)  # torch.Size([8, 3, 224, 224])
    ...
    def compute_stats(self, concept_id, layer_name: str, mode="relevance",
                      top_N=5, mean_N=10, norm=False) -> Tuple[list, list]:
        """
        ...
        for i, t in enumerate(targets):
            ...
            d_c_sorted, rel_c_sorted, rf_c_sorted = load_statistics(path, layer_name, t)
            xx = rel_c_sorted[:mean_N, concept_id]
    ...
    def compute_stats_cls(self, cls_id, layer_name: str, mode="relevance", top_N=5, mean_N=10, norm=False) -> Tuple[list, list]:
        ...
        for i, t in enumerate(targets):
            ...
            d_c_sorted, rel_c_sorted, rf_c_sorted = load_statistics(path, layer_name, t)
            xx = rel_c_sorted[:mean_N, :]

            rel_c_topk = torch.from_numpy(np.mean(xx, axis=0))
            args = torch.argsort(rel_c_topk, descending=True)[:top_N]

            sorted_cid[t] = args.numpy()
            sorted_rel_value[t] = rel_c_topk[args].numpy()

            if norm:
                sorted_rel_value[t] = sorted_rel_value[t] / sorted_rel_value[t][0]

        return sorted_cid, sorted_rel_value
