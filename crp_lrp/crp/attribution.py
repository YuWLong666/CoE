...
from crp_lrp.crp.hooks import MaskHook
from crp_lrp.crp.concepts import Concept, ChannelConcept
from crp_lrp.crp.graph import ModelGraph
...
class CondAttribution:
    ...
    def __init__(self, model: torch.nn.Module,
        ...
        self.MODEL_OUTPUT_NAME = g_y_list.copy()
        if 'y' not in self.MODEL_OUTPUT_NAME:
            self.MODEL_OUTPUT_NAME.append('y')
    ...
    def _check_arguments(self, data, conditions, start_layer, exclude_parallel, init_rel):
        ...
        for cond in conditions:
            b_incond = False
            for i_MODEL_OUTPUT_NAME in self.MODEL_OUTPUT_NAME:
                if i_MODEL_OUTPUT_NAME in cond:
                    b_incond = True

            if b_incond is False and start_layer is None and init_rel is None:
                raise ValueError(
                    f"Either {self.MODEL_OUTPUT_NAME} in 'conditions' or 'start_layer' or 'init_rel' must be defined.")

            if b_incond is True and start_layer is not None:
                warnings.warn(
    ...
    def _conditions_wrapper(self, *args):
        ...
        for dist_layer in dist_conds:
            for i_y_name in dist_conds[dist_layer][0].keys():
                if i_y_name in self.MODEL_OUTPUT_NAME:
                    y_name = i_y_name
            ...
            if heatmap is None:
                ...
                prediction[y_name] = attr.prediction
            else:
    ...
    def _attribute(
        ...
        for i, cond in enumerate(conditions):
            for l_name, indices in cond.items():
                if l_name in self.MODEL_OUTPUT_NAME:
        ...
        with mask_composite.context(self.model), composite.context(self.model) as modified:
            ...
            if start_layer:
                ...
                if self.b_clip is True:
                    pred = modified(data)  # torch.Size([40, 1024])
                else:
                    pred = modified(data)  # torch.Size([40, 1000])
    ...
    def generate(
        ...
        for cond in conditions:
            for l_name in cond.keys():
                if l_name not in hook_map:
                    hook_map[l_name] = MaskHook([])
                if l_name not in self.MODEL_OUTPUT_NAME and l_name not in cond_l_names:
                    cond_l_names.append(l_name)
        ...
        with mask_composite.context(self.model), composite.context(self.model) as modified:
            ...
            if start_layer:
                ...
                logits_fine, logits_coarse, domain_predicted, out_feat_btnk, logits_cnfd ,_,_ = modified(data_batch)
            ...
            for b in range(batches):
                ...
                for i, cond in enumerate(cond_batch):
                    for l_name, indices in cond.items():
                        if l_name in self.MODEL_OUTPUT_NAME:
                            ...
                            self._register_mask_fn(hook_map[l_name], mask_map, i, indices,
                                                   l_name)
                if b == batches - 1:
                ...
                if y_name is not None:
                    if y_name == 'f0':
                        ...
                        pred = logits_coarse[0]
                    elif y_name == 'c2':
                        pred = logits_coarse[1]
                    elif y_name == 'c3':
                        pred = logits_coarse[2]
                    else:
                        pred = logits_fine
                grad_mask = self.relevance_init(pred.detach().clone(), y_targets, init_rel)
