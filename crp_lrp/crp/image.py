...
@torch.no_grad()
def vis_img_heatmap(data_batch, heatmaps, rf=False, crop_th=0.1, kernel_size=19, cmap="bwr", vmin=None, vmax=None,
                    symmetric=True) -> Tuple[Image.Image, Image.Image]:
    """
...
def imgify(image: Union[Image.Image, torch.Tensor, np.ndarray], cmap: str = "bwr", vmin=None, vmax=None,
...
def plot_grid(ref_c: Dict[int, Any], cmap_dim=1, cmap="bwr", vmin=None, vmax=None, symmetric=True,
              resize=None, padding=True, figsize=(6, 6), x_labels=[],
              fig_name='', fig_path='', fontsize=24, b_show=False):
    """
    ...
    for i in range(nrows):
        inner = gridspec.GridSpecFromSubplotSpec(nsubrows, ncols, subplot_spec=outer[i], wspace=0, hspace=0)
        ...
        for sr in range(nsubrows):
            ...
            for c in range(ncols):
                ...
                if sr == cmap_dim:
                    img = imgify(img_list[c], cmap=cmap, vmin=vmin, vmax=vmax, symmetric=symmetric, resize=resize,
                                 padding=padding)
                else:
                ...
                if sr == 0 and c == 0:
                    ax.set_ylabel(keys[i], fontweight='bold', fontsize=fontsize)
